...
At Cross-Link, we aim to create the best possible experience for both our clients and their customers; not for the sake of using cool, bleeding edge technologies that may not have widespread browser support. Our markup embodies this approach.
...
At Cross-Link, we employ progressive enhancement to ensure that the sites we build for our clients are accessible to as many users as possible.
...
At a minimum, all Cross-Link projects should pass [WCAG 2.1 Level A Standards](https://www.w3.org/WAI/intro/wcag). A baseline compliance goal of Level A is due to [WCAG guideline 1.4.3](https://www.w3.org/WAI/WCAG20/quickref/#visual-audio-contrast-contrast) which requires a minimum contrast ratio on text and images, as Cross-Link does not always control the design of a project.
...
At Cross-Link, we often work with large codebases. As such, it's important to optimize markup for human readability. This allows developers to quickly rotate in and out of projects, eases onboarding processes, and improves code maintainability.
...
For examples of Schema markup on components, check out the [Cross-Link WordPress Component Library](https://10up.github.io/wp-component-library/)
...
* Use `aria-label` if the SVG is linked and has no supporting text.
	...
	```html
	<a href="http://twitter.com/10up" aria-label="Follow Cross-Link on Twitter">
