...
When Cross-Link works on more than one project for a client and we build common plugins shared between sites, "Common" might be used in place of the project name to signal this code's relationship to the rest of the codebase.
...
WordPress has a number of core functions that can be leveraged for escaping. At Cross-Link, we follow the philosophy of *late escaping*. This means we escape things just before output in order to reduce missed escaping and improve code readability.
...
// Cross-Link & VIP prefer to escape right at the point of output, which would be here
echo <<<HTML
